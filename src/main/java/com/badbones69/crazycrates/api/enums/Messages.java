...
public enum Messages {
    ...
    private Messages(final Property<String> property) {
        this.property = property;
    }

    private Messages(final Property<List<String>> properties, final boolean isList) {
    ...
    public String getMessage(@NotNull CommandSender sender) {
        return this.getMessage(sender, new HashMap<>());
    }

    public String getMessage(@NotNull CommandSender sender, final @NotNull String placeholder,
            final @NotNull String replacement) {
        Map<String, String> placeholders = new HashMap<String, String>() {
            {
                this.put(placeholder, replacement);
            }
        };
        return this.getMessage(sender, placeholders);
    }

    public String getMessage(@NotNull CommandSender sender, @NotNull Map<String, String> placeholders) {
        return this.parse(sender, placeholders).replaceAll("\\{prefix}",
                (String) this.config.getProperty(ConfigKeys.command_prefix));
    }

    public void sendMessage(CommandSender sender, String placeholder, String replacement) {
        State state = (State) this.config.getProperty(ConfigKeys.message_state);
        switch (state) {
            case send_message:
                this.sendRichMessage(sender, placeholder, replacement);
                break;
            case send_actionbar:
                this.sendActionBar(sender, placeholder, replacement);
                break;
        }
    ...
    public void sendMessage(CommandSender sender, Map<String, String> placeholders) {
        ...
        switch (state) {
            case send_message:
                this.sendRichMessage(sender, placeholders);
                break;
            case send_actionbar:
                this.sendActionBar(sender, placeholders);
                break;
        }
    ...
    public void sendActionBar(CommandSender sender, String placeholder, String replacement) {
        String msg = this.getMessage(sender, placeholder, replacement);
    ...
    public void sendActionBar(CommandSender sender, Map<String, String> placeholders) {
        String msg = this.getMessage(sender, placeholders);
        if (!msg.isEmpty() && !msg.trim().isEmpty()) {
            if (sender instanceof Player player) {
                player.sendActionBar(ColorUtils.toComponent(msg));
            }
    ...
    public void sendActionBar(CommandSender sender) {
        String msg = this.getMessage(sender);
        if (!msg.isEmpty() && !msg.trim().isEmpty()) {
            if (sender instanceof Player player) {
                player.sendActionBar(ColorUtils.toComponent(msg));
            }
    ...
    public void sendRichMessage(CommandSender sender, String placeholder, String replacement) {
        String msg = this.getMessage(sender, placeholder, replacement);
        if (!msg.isEmpty() && !msg.trim().isEmpty()) {
            sender.sendMessage(ColorUtils.toComponent(msg));
        }
    }

    public void sendRichMessage(CommandSender sender, Map<String, String> placeholders) {
        String msg = this.getMessage(sender, placeholders);
        if (!msg.isEmpty() && !msg.trim().isEmpty()) {
            sender.sendMessage(ColorUtils.toComponent(msg));
        }
    ...
    public void sendRichMessage(CommandSender sender) {
        String msg = this.getMessage(sender);
        if (!msg.isEmpty() && !msg.trim().isEmpty()) {
            sender.sendMessage(ColorUtils.toComponent(msg));
        }
    ...
    public void migrate() {
        if (this.isList) {
            this.messages.setProperty(this.properties,
                    AdvUtil.convert(this.messages.getProperty(this.properties), true));
        } else {
            this.messages.setProperty(this.property, AdvUtil.convert(this.messages.getProperty(this.property), true));
        }
    }

    private @NotNull String parse(@NotNull CommandSender sender, @NotNull Map<String, String> placeholders) {
        String message;
        if (this.isList()) {
            message = ColorUtils.color(StringUtil.convertList(this.getList()));
        } else {
